...
def validacion(cadena):
    ...
    for i in range(npal):
        ...
        n = len(palabras[i]) - 1
        if i > 0:
            n1 = len(palabras[i-1]) - 1
            k = 0
            while k < len(palabras[i-1]):  # cuenta cuantos puntos tiene la palabra anterior
                if palabras[i-1].__getitem__(k) == '.':
                    p += 1
                k += 1
        ...
        while h < len(palabras[i]):
            ...
            if i > 0 and p == 1 and palabras[i - 1].__getitem__(n1) == '.':  # Indica si la palabra anterior tenía un punto
                flgptr = True
                #print("Bandera activa")
            if i > 0 and palabras[i - 1].__getitem__(n1) != '.':  # Indica si la palabra anterior no tenía un punto
                flgptr = False
